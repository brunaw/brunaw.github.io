...
- Firstly presented in `r Cite(bibs[key = "guided"])`:
  - The authors **penalise the gain (RSS reduction)** of each 
  variable, for each tree when building a mdoel
...
```{r
mse <- function(model, test, formula = ridership ~ .,
us_hol = timeDate::listHolidays() %>% str_subset("(^US)|(Easter)")){
...
results <- training_models %>% 
  mutate(mse = map2_dbl(.x = model_trained, .y = map(train_test, testing), mse), 
...
tab <- results %>% 
  select(model, mse, n_variables, rsquared) %>% 
  group_by(model) %>% 
  summarise_all(mean) %>% 
  arrange(model)
